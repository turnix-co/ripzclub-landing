      ...
      <div className="relative rounded-[28px] border border-white/12 bg-[rgba(16,18,28,.55)] px-[clamp(24px,4vw,56px)] pb-[clamp(36px,4vw,52px)] pt-[clamp(44px,5vw,64px)] shadow-[0_40px_90px_rgba(0,0,0,.55)] backdrop-blur-[18px]">
        ...
        <div className="absolute left-1/2 top-0 flex -translate-x-1/2 -translate-y-1/2 items-center">
          ...
          <span
            ...
            style={{ clipPath: "polygon(0 0, 100% 0, 100% 100%, 0 100%, 16px 50%)" }}
          />
          {/* center plate */}
            ...
            {done ? (
              <div className="flex min-h-[320px] flex-col items-center justify-center gap-4 text-center">
                <div className="flex size-[76px] items-center justify-center rounded-full border-2 border-[#22c55e] bg-[#22c55e]/15">
                  <svg viewBox="0 0 24 24" fill="none" stroke="#22c55e" strokeWidth={3} strokeLinecap="round" strokeLinejoin="round" className="size-10">
                    <path d="M4 12.5 10 18.5 20 6.5" />
                  </svg>
                </div>
                <h2 className="font-poppins text-[28px] font-bold text-paper">You&apos;re on the list!</h2>
                <p className="font-poppins text-sm text-paper/60">
                  Watch your inbox — you&apos;re entered in this month&apos;s draw.
                </p>
              </div>
            ) : (
              <form ref={formRef} onSubmit={submit} noValidate>
                ...
                <p className="mb-[22px] font-poppins text-sm text-paper/62">
                  Join the list, lock your spot, and you&apos;re automatically in this month&apos;s prize draw.
                </p>

                <div className="grid grid-cols-2 gap-3">
                  <div>
                    <label className={labelClass} htmlFor="first">First name</label>
                    <input id="first" name="first" placeholder="Jordan" autoComplete="given-name" className={fieldClass} />
                  </div>
                  <div>
                    <label className={labelClass} htmlFor="last">Last name</label>
                    <input id="last" name="last" placeholder="Carter" autoComplete="family-name" className={fieldClass} />
                  </div>
                </div>

                <div className="mt-3">
                  <label className={labelClass} htmlFor="email">Email address</label>
                  <input ref={emailRef} id="email" name="email" type="email" required placeholder="you@email.com" autoComplete="email" className={fieldClass} />
                </div>
                ...
                <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div className="relative">
                    <span className="absolute left-[7px] top-1/2 flex size-[38px] -translate-y-1/2 items-center justify-center rounded-[9px] bg-[linear-gradient(45deg,#feda75,#fa7e1e,#d62976,#962fbf,#4f5bd5)]">
                      <svg viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth={2} className="size-5">
                        <rect x="3.5" y="3.5" width="17" height="17" rx="5" />
                        <circle cx="12" cy="12" r="4" />
                        <circle cx="17.2" cy="6.8" r="1.1" fill="#fff" stroke="none" />
                      </svg>
                    </span>
                    <input name="instagram" placeholder="Instagram handle" aria-label="Instagram handle" className={`${fieldClass} pl-[54px]`} />
                  </div>
                  <div className="relative">
                    ...
                    <input name="discord" placeholder="Discord username" aria-label="Discord username" className={`${fieldClass} pl-[54px]`} />
                  </div>
                </div>
